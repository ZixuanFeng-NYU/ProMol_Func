...
def run_stat(args: Namespace) -> Tuple[list, list]:
    ...
    print("len(model_preds)", len(model_preds))
    test_path=args.test_path
    #target=test_path.split('/')[-3]
...
if __name__ == '__main__':
    ...
    for target in targets:
        if target:
            Class.append(target[0])
        else:
            Class.append(None)
    df_out['Class']=Class
    df_out['preds'] = [pred[0] for pred in preds]
    df_out['preds']=df_out['preds'].astype(float)
    df_out['preds_binary'] = df_out['preds'] > 0.5
    df_out['preds_binary']=df_out['preds_binary'].astype(int)
    save_path=args.preds_path
    df_out.to_csv(os.path.join(save_path,f'{file_part}_ProMol_Func_prediction.csv'))
    #original=pd.read_csv(args.test_path)
    #original['KANO_preds_binary']=df_out['KANO_preds_binary']
    #common=df_out
    #accuracy=accuracy_score(common['Class'],common['KANO_preds_binary'])
    #print("accuracy:",accuracy)
    #precision=precision_score(common['Class'],common['KANO_preds_binary'])
    #print("precision:",precision)
    #recall=recall_score(common['Class'],common['KANO_preds_binary'])
    #print("recall:",recall)

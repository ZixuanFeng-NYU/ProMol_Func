...
# =============================================================================
        ...
        # Get the class labels for each molecule
        class_labels = np.array([data[i].targets[0] for i in range(len(data))])

        # Perform stratified splitting using train_test_split function
        train_val_indices, test_indices = train_test_split(range(len(data)), test_size=sizes[2], stratify=class_labels, random_state=seed)
        train_indices, val_indices = train_test_split(train_val_indices, test_size=sizes[1] / (sizes[0] + sizes[1]), stratify=class_labels[train_val_indices], random_state=seed)

        # Create train, validation, and test datasets based on the indices
    ...
    elif split_type == 'stratified_separate_test':
        print(data)
        ...
        # Perform stratified splitting using train_test_split function
        #train_val_indices, test_indices = train_test_split(range(len(data)), test_size=sizes[2], stratify=class_labels, random_state=1)
        train_indices, val_indices = train_test_split(range(len(data)), test_size=sizes[1] / (sizes[0] + sizes[1]), stratify=class_labels, random_state=seed)

...
#ef attention(query, key, value, mask, dropout=None):
...
class CMPN(nn.Module):
    ...
    def forward(self, step, prompt: bool, smiles_batch, pro_id_batch,
        ...
        pro_functions_list = []
        if pro_id_batch is not None:
            for pro_id in pro_id_batch:
                with open(f"../DeepFRI/DeepFRI_outputs/{pro_id}_MF_pred_scores.json",'r') as file:

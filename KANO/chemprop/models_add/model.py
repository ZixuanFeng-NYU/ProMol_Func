...
class MoleculeModel(nn.Module):
    ...
    def forward(self, *input):
        ...
        if not self.pretrain:
            cur_encoder=self.encoder(*input)
            small_molecules_output = cur_encoder[0]
            pro_func_tensor=cur_encoder[1]
            pro_func_tensor=self.linear(pro_func_tensor)
            combined_output=small_molecules_output+pro_func_tensor
            output=self.ffn(combined_output)

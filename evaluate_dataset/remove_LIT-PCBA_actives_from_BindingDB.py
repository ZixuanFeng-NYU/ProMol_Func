...
df = pd.read_csv("LIT-PCBA_protein_ligand_data.csv")
df_positives=df[df['Class']==1.0]
print(df_positives)
# Find duplicated proteins based on sequence
duplicated_protein = df_BindingDB[df_BindingDB["BindingDB Target Chain Sequence"].isin(df_positives['Sequence'])]
...
duplicated_protein_duplicated_smiles = duplicated_protein[duplicated_protein.apply(
    lambda row: any(is_identical_smiles(row['Ligand SMILES'], smiles) for smiles in df_positives[df_positives['Sequence'] == row['BindingDB Target Chain Sequence']]['SMILES']),
    axis=1
)]
